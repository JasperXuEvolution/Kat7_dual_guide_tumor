...
Change:
    Because Jackie use 27 bp gRNA for spikein position 2. So i changed this code.
"""
...
def main():
    ...
    temp_pattern1_spikein = regex.compile('TATATAGTT(.{16})TATG(.{16,21})GTTTA')
    # Pattern for read2: Extract a sgRNA (16-21 bp) from the reverse complemented read.
    temp_pattern2 = regex.compile('TGTTG(.{16,27})GTTTG')
    ...
    with gzip.open(fastqgz_input_address1, 'rt') as handler1, gzip.open(fastqgz_input_address2, 'rt') as handler2:
        ...
        while read_id:
            ...
            if match2:
                ...
                if  gRNA2 == 'ACTAGCAGGTGACACCACCTGCATCA': # if it is spikein read
                    match1 = temp_pattern1_spikein.search(seq1)
                else:
                ...
                if match1:
                    ...
                    failed_match1_only += 1  # match2 ok, match1 failed
            else:
                failed_match2 += 1  # match2 failed
    ...
    match2_work = total_reads - failed_match2  # match1+match2 both failed
    print(f"Failed read breakdown:")
    print(f"- {failed_match2} reads ({failed_match2/total_reads:.3%}) failed at match2 (R2 gRNA) out of total")
    print(f"- {failed_match1_only} reads ({failed_match1_only/match2_work:.3%}) failed at match1 out of all match2 success")
